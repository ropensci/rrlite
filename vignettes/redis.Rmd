...
All the commands are the same; in fact the only way of telling if you have an `rlite` or an `Redis` based interface (currently) is to inspect the `context` member:

```{r}
r1$context # rlite_context
r2$context # redis_context
```
